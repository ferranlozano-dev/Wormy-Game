...
import cat.flx.plataformes.game.characters.PrinPrinEnemigo;
import cat.flx.plataformes.game.characters.Teleport;
import cat.flx.plataformes.game.characters.Velocidad;
import cat.flx.plataformes.game.characters.Vida;
...
class Scene01 extends TiledScene implements OnContactListener, GameObject.OnTouchEventListener {
    ...
    Scene01(Game game) {
        ...
        this.loadFromFile(R.raw.sc_ferran_lozano);
        ...
        this.addContactListener("bonk", "vida", this);
        this.addContactListener("bonk", "velocidad", this);
    ...
    protected GameObject parseLine(String cmd, String args) {
        ...
        if (cmd.equals("VIDA")) {
            ...
            int coinX = Integer.parseInt(parts2[0].trim()) * 16;
            int coinY = Integer.parseInt(parts2[1].trim()) * 16;
            return new Vida(game, coinX, coinY);
        }
        if (cmd.equals("VELOCIDAD")) {
            String[] parts2 = args.split(",");
            if (parts2.length != 2) return null;
            int coinX = Integer.parseInt(parts2[0].trim()) * 16;
            int coinY = Integer.parseInt(parts2[1].trim()) * 16;
            return new Velocidad(game, coinX, coinY);
        }
        ...
        if (cmd.equals("PRINPRINENEMY")) {
            String[] parts2 = args.split(",");
            if (parts2.length != 3) return null;
            int prinX0 = Integer.parseInt(parts2[0].trim()) * 16;
            int prinX1 = Integer.parseInt(parts2[1].trim()) * 16;
            int prinY = Integer.parseInt(parts2[2].trim()) * 16 + 2;
            return new PrinPrinEnemigo(game, prinX0, prinX1, prinY);
        }
        // Lines beginning with "TPORT"
        if (cmd.equals("TPORT")) {
            String[] parts2 = args.split(",");
            if (parts2.length != 4) return null;
            int tportX0 = Integer.parseInt(parts2[0].trim()) * 16;
            int tportY0 = Integer.parseInt(parts2[1].trim()) * 16;
            int tportX1 = Integer.parseInt(parts2[2].trim()) * 16;
            int tportY1 = Integer.parseInt(parts2[3].trim()) * 16;
            return new Teleport(game, tportX0, tportY0, tportX1, tportY1);
        }
    ...
    public void onContact(String tag1, GameObject object1, String tag2, GameObject object2) {
        ...
        switch (tag2) {
            ...
            case "prinprin":
                game.loadScene(new YouWin(game));

                break;
            case "teleport":
                Teleport teleport = (Teleport) object2;
                int x = teleport.getDestinationX();
                int y = teleport.getDestinationY();
                bonk.setPosition(x, y);
                break;
        }
    }
